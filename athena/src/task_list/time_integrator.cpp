...
TimeIntegratorTaskList::TimeIntegratorTaskList(ParameterInput *pin, Mesh *pm) {
  ...
  {using namespace HydroIntegratorTaskNames; // NOLINT (build/namespace)
    ...
    if (ORBITAL_ADVECTION) {
      ...
      // TODO(sxc18): implement for orbital advection for particles
      if (PARTICLES > 0)
        AddTask(SEND_HYD, SRCTERM_HYD | SETB_PM);
      else
    ...
    if (PARTICLES > 0) {
      ...
      AddTask(SETB_PM, (RECV_PM | INT_PAR));
      AddTask(RECV_NUM_PAR, NONE);
      AddTask(RECV_PAR, NONE);
      AddTask(SETB_PAR, (RECV_PAR | SEND_PAR));
    }
...
void TimeIntegratorTaskList::AddTask(const TaskID& id, const TaskID& dep) {
  ...
  if (id == CLEAR_ALLBND) {
    ...
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_NUM_PAR) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::ParticlesNumReceive);
    task_list_[ntasks].lb_time = false;
  } else if (id == RECV_PAR) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
    ...
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::ParticlesSetBoundaries);
    task_list_[ntasks].lb_time = true;
  } else if (id == SEND_PM) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
    ...
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::ParticleMeshSetBoundaries);
    task_list_[ntasks].lb_time = true;
  } else {
    std::stringstream msg;
...
void TimeIntegratorTaskList::StartupTaskList(MeshBlock *pmb, int stage) {
  ...
  if (stage_wghts[stage-1].main_stage) {
    ...
    if (PARTICLES > 0) pmb->ppar->InitRecvPar(BoundaryCommSubset::all);
    // Update coordinate parameters
    pmb->pcoord->UpdateCoords(
        pmb->pmy_mesh->time +
            stage_wghts[(stage - 1)].ebeta * pmb->pmy_mesh->dt -
            (stage_wghts[(stage - 1)].beta) * (pmb->pmy_mesh->dt),
        (stage_wghts[(stage - 1)].beta) * (pmb->pmy_mesh->dt));
  } else {
...
TaskStatus TimeIntegratorTaskList::ClearAllBoundary(MeshBlock *pmb, int stage) {
  if (stage_wghts[stage-1].main_stage) {
    pmb->pbval->ClearBoundarySubset(BoundaryCommSubset::all,
                                    pmb->pbval->bvars_main_int);
    if (PARTICLES > 0) pmb->ppar->ClearParBoundary(BoundaryCommSubset::all);
  } else {
...
TaskStatus TimeIntegratorTaskList::IntegrateHydro(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage) {
      ...
      pmb->pcoord->AddCoordTermsDivergence(CoordTermsDivergence::stationary,
                                           wght, ph->flux, ph->w, pf->bcc,
                                           ph->u);
...
TaskStatus TimeIntegratorTaskList::IntegrateField(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage) {
      ...
      // add coordinate (geometric) source terms
      pmb->pcoord->AddCoordTermsCT(
          CoordTermsDivergence::stationary,
          stage_wghts[stage - 1].beta * pmb->pmy_mesh->dt, pf->bcc, pf->b);
    }
    return TaskStatus::next;
  }
  return TaskStatus::fail;
  ...
  if (stage <= nstages) {
    ...
    if (std::strcmp(COORDINATE_SYSTEM, "expanding") == 0) {
      if (NON_BAROTROPIC_EOS && MAGNETIC_FIELDS_ENABLED)
        pmb->pfield->CalculateCellCenteredField(pf->b, pf->bcc, pmb->pcoord, il,
                                                iu, jl, ju, kl, ku);
      pmb->pcoord->AddCoordTermsDivergence(CoordTermsDivergence::preliminary,
                                           pmb->pmy_mesh->dt, ph->flux, ph->w,
                                           pf->bcc, ph->u);
      if (MAGNETIC_FIELDS_ENABLED)
        pmb->pcoord->AddCoordTermsCT(CoordTermsDivergence::finale,
                                     pmb->pmy_mesh->dt, pf->bcc, pf->b);
      if (NON_BAROTROPIC_EOS && MAGNETIC_FIELDS_ENABLED)
        pmb->pfield->CalculateCellCenteredField(pf->b, pf->bcc, pmb->pcoord, il,
                                                iu, jl, ju, kl, ku);
      pmb->pcoord->AddCoordTermsDivergence(CoordTermsDivergence::finale,
                                           pmb->pmy_mesh->dt, ph->flux, ph->w,
                                           pf->bcc, ph->u);
    }
...
TaskStatus TimeIntegratorTaskList::ParticlesIntegrate(MeshBlock *pmb,
                                                           int stage) {
  if (integrator == "vl2") {
...
TaskStatus TimeIntegratorTaskList::ParticlesSend(MeshBlock *pmb,
                                                      int stage) {
  pmb->ppar->SendParticles();
  return TaskStatus::success;
}

TaskStatus TimeIntegratorTaskList::ParticlesReceive(MeshBlock *pmb,
                                                         int stage) {
  bool ret;
  if (stage <= nstages) {
    ret = pmb->ppar->ReceiveParticles();
  } else {
    return TaskStatus::fail;
  }
  if (ret) {
...
TaskStatus TimeIntegratorTaskList::ParticlesNumReceive(MeshBlock *pmb, int stage) {
  bool ret;
  if (stage <= nstages) {
    ret = pmb->ppar->ReceiveNumParAndStartRecv();
  } else {
    return TaskStatus::fail;
  }
  if (ret) {
...
TaskStatus TimeIntegratorTaskList::ParticlesSetBoundaries(MeshBlock *pmb,
                                                          int stage) {
  if (stage <= nstages) {
    pmb->ppar->SetParticles();
    return TaskStatus::success;
  }
  return TaskStatus::fail;
}

TaskStatus TimeIntegratorTaskList::ParticleMeshSend(MeshBlock *pmb,
                                                         int stage) {
  if (stage <= nstages) {
...
TaskStatus TimeIntegratorTaskList::ParticleMeshReceive(MeshBlock *pmb,
                                                            int stage) {
  bool ret;
  if (stage <= nstages) {
    ret = pmb->ppar->ReceiveParticleMesh();
  } else {
    return TaskStatus::fail;
  }
  if (ret) {
...
TaskStatus TimeIntegratorTaskList::ParticleMeshSetBoundaries(MeshBlock *pmb,
                                                             int stage) {
  if (stage <= nstages) {
    pmb->ppar->SetParticleMesh(stage);
    return TaskStatus::success;
  }
  return TaskStatus::fail;
}