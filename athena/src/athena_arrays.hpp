...
class AthenaArray {
 public:
  ...
  bool IsShallowSlice() const { return (state_ == DataStatus::shallow_slice); }
  bool IsEmpty() const { return (state_ == DataStatus::empty); }
  bool IsAllocated() const { return (state_ == DataStatus::allocated); }
...
#pragma omp declare simd simdlen(SIMD_WIDTH) linear(n)
...
#pragma omp declare simd simdlen(SIMD_WIDTH)
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(n) linear(i)
...
#pragma omp declare simd simdlen(SIMD_WIDTH)
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(n, j) linear(i)
...
#pragma omp declare simd simdlen(SIMD_WIDTH)
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(n, j, k) linear(i)
...
#pragma omp declare simd simdlen(SIMD_WIDTH)
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(n, j, k, m) linear(i)
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(n, j, k, m) linear(i)
  ...
  // int l?, int o?
#pragma omp declare simd simdlen(SIMD_WIDTH)
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(n, j, k, m, p) linear(i)
...
void AthenaArray<T>::ResizeLastDimension(int new_nx1) {
  ...
  nx1_ = new_nx1;
  if (new_nx1 == 0) {
    if (state_ != DataStatus::shallow_slice && state_ != DataStatus::empty)
      delete[] pdata_;
    pdata_ = nullptr;
    state_ = DataStatus::empty;
    return;
  ...
  if (state_ == DataStatus::empty) {
    NewAthenaArray(nx5_, nx4_, nx3_, nx2_, nx1_);
    return;
  }

  std::size_t size(std::min(old_nx1, new_nx1) * sizeof(pdata_[0]));
    ...
    for (int ix4 = 0; ix4 < nx4_; ++ix4)
      for (int ix3 = 0; ix3 < nx3_; ++ix3)
        for (int ix2 = 0; ix2 < nx2_; ++ix2) {
          ...
          std::memcpy(pdata_ + jnew, pdata_old + jold, size);
          jold += old_nx1;
          jnew += new_nx1;
        }
