...
SphericalPolar::SphericalPolar(MeshBlock *pmb, ParameterInput *pin, bool flag)
  ...
  if (pmb->block_size.nx3 == 1) {
    ...
    phi_pole_up = 0;
    theta_pole = 0;
    func_cross_pole = [](const Real &phi) { return false; };
  } else {
    ...
    nc_2PI = static_cast<int>(TWO_PI / dx3v(kl));  // Assuming a uniform mesh
    if (IsPole(jl)) { // inner boundary
      phi_pole_low = x3f(kl - ng) + PI;
      phi_pole_up = x3f(ku + ng + 1) + PI;
      ...
      theta_pole = 0.0;
    } else if (IsPole(ju + 1)) { // outer bourndary
      phi_pole_low = x3f(kl - ng) + PI;
      phi_pole_up = x3f(ku + ng + 1) + PI;
      if (phi_pole_low > TWO_PI) phi_pole_low -= TWO_PI;
      if (phi_pole_up > TWO_PI) phi_pole_up -= TWO_PI;
...
void SphericalPolar::AddCoordTermsDivergence(const CoordTermsDivergence &flag,
                                             const Real dt,
...
void SphericalPolar::AddCoordTermsDivergence_STS(
    const CoordTermsDivergence &flag, const Real dt, int stage,
...
inline void SphericalPolar::CartesianToMeshCoords(const Real &x, const Real &y,
                                                  const Real &z, Real &x1,
...
inline void SphericalPolar::MeshCoordsToCartesian(const Real &x1,
                                                  const Real &x2,
                                                  const Real &x3, Real &x,
                                                  Real &y, Real &z) const {
...
//--------------------------------------------------------------------------------------
//! \fn void SphericalPolar::MeshCoordsToIndices(Real x1, Real x2, Real x3,
//! Real& xi1, Real& xi2, Real& xi3) const
//  \brief returns in index coordinates (xi1, xi2, xi3) with respect to the
//  local grid of MeshBlock pmb from the physical coordinates (x1, x2, x3).
//  Additionally taking care the azimuthal direction, remapping points in the
//  ghost zone when they cross the polar.
//  TODO(ccyang): Currently only supports uniform mesh.

inline void SphericalPolar::MeshCoordsToIndices(const Real &x1, const Real &x2,
                                                const Real &x3, Real &xi1,
                                                Real &xi2, Real &xi3) const {
  Real theta(x2), phi(x3);
  if (func_cross_pole(phi)) {
    phi -= PI;
    if (phi < 0) phi += TWO_PI;
    theta = theta_pole - theta;
  }
  Coordinates::MeshCoordsToIndices(x1, theta, phi, xi1, xi2, xi3);
  if (xi3 > nc3)
    xi3 -= nc_2PI;
  else if (xi3 < 0)
    xi3 += nc_2PI;
...
inline void SphericalPolar::CartesianToMeshCoordsVector(
...
inline void SphericalPolar::MeshCoordsToCartesianVector(
