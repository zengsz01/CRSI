...
void Coordinates::UpdateCoords(const Real &time, const Real &dt) { return; }
...
void Coordinates::AddCoordTermsDivergence(const CoordTermsDivergence &flag,
                                          const Real dt,
...
void Coordinates::AddCoordTermsDivergence_STS(const CoordTermsDivergence &flag,
...
//----------------------------------------------------------------------------------------
// Coordinate (Geometric) source term function for field
void Coordinates::AddCoordTermsCT(const CoordTermsDivergence &flag,
                                  const Real dt, const AthenaArray<Real> &bcc,
                                  FaceField &b_out) {
  return;
}

void Coordinates::AddCoordTermsPar(const CoordTermsDivergence &flag,
...
inline void Coordinates::MeshCoordsToIndices(const Real &x1, const Real &x2,
...
inline void Coordinates::IndicesToMeshCoords(Real xi1, Real xi2, Real xi3,
                                             Real &x1, Real &x2,
                                             Real &x3) const {
  // Get the meshblock info.
  const int is = pmy_block->is;
  ...
  // Make the conversion.
  x1 =
      (block_size.nx1 > 1) ? block_size.x1min + (xi1 - is) * dx1f(is) : x1v(is);
  x2 =
      (block_size.nx2 > 1) ? block_size.x2min + (xi2 - js) * dx2f(js) : x2v(js);
  x3 =
      (block_size.nx3 > 1) ? block_size.x3min + (xi3 - ks) * dx3f(ks) : x3v(ks);
}

//----------------------------------------------------------------------------------------
// Stub functions.

inline void Coordinates::CartesianToMeshCoords(const Real &x, const Real &y,
...
inline void Coordinates::MeshCoordsToCartesian(const Real &x1, const Real &x2,
...
inline void Coordinates::CartesianToMeshCoordsVector(
...
inline void Coordinates::MeshCoordsToCartesianVector(
