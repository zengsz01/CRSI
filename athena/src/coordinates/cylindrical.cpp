...
Cylindrical::Cylindrical(MeshBlock *pmb, ParameterInput *pin, bool flag)
  ...
  if (pmb->block_size.nx2 == 1) {
    ...
    nc_2PI = static_cast<int>(TWO_PI / dx2v(jl));  // Assuming a uniform mesh
  }
...
void Cylindrical::AddCoordTermsDivergence(const CoordTermsDivergence &flag,
                                          const Real dt,
...
void Cylindrical::AddCoordTermsDivergence_STS(const CoordTermsDivergence &flag,
...
inline void Cylindrical::CartesianToMeshCoords(const Real &x, const Real &y,
...
inline void Cylindrical::MeshCoordsToCartesian(const Real &x1, const Real &x2,
...
//--------------------------------------------------------------------------------------
//! \fn void Cylindrical::MeshCoordsToIndices(Real x1, Real x2, Real x3, Real&
//! xi1, Real& xi2, Real& xi3) const
...
//  ghost zone when they cross theta = 0.
// TODO(ccyang): Currently only supports uniform mesh.

inline void Cylindrical::MeshCoordsToIndices(const Real &x1, const Real &x2,
                                             const Real &x3, Real &xi1,
                                             Real &xi2, Real &xi3) const {
  Coordinates::MeshCoordsToIndices(x1, x2, x3, xi1, xi2, xi3);
  // When locating at the ghost zone in x2 dir
  if (xi2 > nc2)
    xi2 -= nc_2PI;
  else if (xi2 < 0)
    xi2 += nc_2PI;
...
inline void Cylindrical::CartesianToMeshCoordsVector(
    const Real &x, const Real &y, const Real &z, const Real &vx, const Real &vy,
    const Real &vz, Real &vx1, Real &vx2, Real &vx3) const {
  Real z_tmp = std::sqrt(x * x + y * y);
  Real x_tmp(x / (z_tmp > 0 ? z_tmp : 1));
  Real y_tmp(y / (z_tmp > 0 ? z_tmp : 1));
  vx1 = vx * x_tmp + vy * y_tmp;
  vx2 = vy * x_tmp - vx * y_tmp;
  vx3 = vz;
}
...
inline void Cylindrical::MeshCoordsToCartesianVector(
    const Real &x, const Real &y, const Real &z, const Real &vx1,
    const Real &vx2, const Real &vx3, Real &vx, Real &vy, Real &vz) const {
  Real x1(std::cos(y)), x2(std::sin(y));
  vx = vx1 * x1 - vx2 * x2;
  vy = vx1 * x2 + vx2 * x1;
  vz = vx3;
}
