...
//------------------
enum class CoordTermsDivergence { stationary, preliminary, finale };
...
class Coordinates {
 public:
  ...
  // functions...
  // functions to update coordiante with time
  virtual void UpdateCoords(const Real &time, const Real &dt);
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this, k, j) linear(i)
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this, k, j) linear(i)
  ...
  virtual void AddCoordTermsDivergence(const CoordTermsDivergence &flag,
                                       const Real dt,
                                       const AthenaArray<Real> *flux,
                                       const AthenaArray<Real> &prim,
                                       const AthenaArray<Real> &bcc,
                                       AthenaArray<Real> &u);
  virtual void AddCoordTermsDivergence_STS(const CoordTermsDivergence &flag,
                                           const Real dt, int stage,
                                           const AthenaArray<Real> *flux,
                                           AthenaArray<Real> &u,
                                           AthenaArray<Real> &flux_div);
  virtual void AddCoordTermsCT(const CoordTermsDivergence &flag, const Real dt,
                               const AthenaArray<Real> &bcc,
                               FaceField &b_out);  // Field
  virtual void AddCoordTermsPar(const CoordTermsDivergence &flag,
                                const Real &time, const Real dt,
                                const AthenaArray<Real> &meshsrc,
                                const int &npar, AthenaArray<Real> &v1,
                                AthenaArray<Real> &v2,
                                AthenaArray<Real> &v3);  // Particle
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this) linear(ref(x, y, z))
  virtual void CartesianToMeshCoords(const Real &x, const Real &y,
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this) linear(ref(x, y, z))
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this) \
    linear(ref(x1, x2, x3, x, y, z))
  virtual void MeshCoordsToCartesian(const Real &x1, const Real &x2,
                                     const Real &x3, Real &x, Real &y,
                                     Real &z) const;
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this)
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this) \
    linear(ref(xi1, xi2, xi3))
  virtual void MeshCoordsToIndices(const Real &x1, const Real &x2,
                                   const Real &x3, Real &xi1, Real &xi2,
                                   Real &xi3) const;
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this)
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this, xi3, xi2) \
    linear(ref(x1, x2, x3))
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this) \
    linear(ref(x1, x2, x3))
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this) linear(ref(x, y, z))
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this) \
    linear(ref(vx1, vx2, vx3))
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this) \
    linear(ref(vx, vy, vz))
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this) \
    linear(ref(vx1, vx2, vx3, vx, vy, vz))
  virtual void CartesianToMeshCoordsVector(const Real &x, const Real &y,
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this) \
    linear(ref(vx1, vx2, vx3, vx, vy, vz))
  virtual void MeshCoordsToCartesianVector(const Real &x1, const Real &x2,
                                           const Real &x3, const Real &vx1,
 ...
 protected:
  ...
  int nc_2PI;                      // number of cells crossing total 2Pi, using in x2 of 
                                   // cylindrical coords andx3 of spherical coords
  // angle range when crossing the pole, used in the spherical coordes
  Real phi_pole_low, phi_pole_up, theta_pole;
...
class Cartesian : public Coordinates {
 ...
 public:
  ...
  void CartesianToMeshCoords(const Real &x, const Real &y, const Real &z,
                             Real &x1, Real &x2, Real &x3) const final;
  void MeshCoordsToCartesian(const Real &x1, const Real &x2, const Real &x3,
                             Real &x, Real &y, Real &z) const final;
  void CartesianToMeshCoordsVector(const Real &x, const Real &y, const Real &z,
                                   const Real &vx, const Real &vy,
                                   const Real &vz, Real &vx1, Real &vx2,
                                   Real &vx3) const final;
  void MeshCoordsToCartesianVector(const Real &x1, const Real &x2,
                                   const Real &x3, const Real &vx1,
                                   const Real &vx2, const Real &vx3, Real &vx,
                                   Real &vy, Real &vz) const final;
};
...
class Cylindrical : public Coordinates {
 ...
 public:
  ...
  // ...to compute geometrical source terms
  void AddCoordTermsDivergence(const CoordTermsDivergence &flag, const Real dt,
                               const AthenaArray<Real> *flux,
                               const AthenaArray<Real> &prim,
                               const AthenaArray<Real> &bcc,
                               AthenaArray<Real> &u) final;
  void AddCoordTermsDivergence_STS(const CoordTermsDivergence &flag,
                                   const Real dt, int stage,
                                   const AthenaArray<Real> *flux,
                                   AthenaArray<Real> &u,
  ...
  void MeshCoordsToCartesian(const Real &x1, const Real &x2, const Real &x3,
                             Real &x, Real &y, Real &z) const final;
  void MeshCoordsToIndices(const Real &x1, const Real &x2, const Real &x3,
                           Real &xi1, Real &xi2, Real &xi3) const final;
  void CartesianToMeshCoordsVector(const Real &x, const Real &y, const Real &z,
                                   const Real &vx, const Real &vy,
                                   const Real &vz, Real &vx1, Real &vx2,
                                   Real &vx3) const final;
...
//----------------------------------------------------------------------------------------
//! \class ExpandingCoord
//  \brief derived class for expanding coordinates.  None of the virtual funcs
//  in the Coordinates abstract base class need to be overridden.

class ExpandingCoord : public Coordinates {
  friend class HydroSourceTerms;

 public:
  ExpandingCoord(MeshBlock *pmb, ParameterInput *pin, bool flag);

  // functions to update coordiante with time
  void UpdateCoords(const Real &time, const Real &dt) final;
  ...
  void AddCoordTermsDivergence(const CoordTermsDivergence &flag, const Real dt,
                               ...
                               const AthenaArray<Real> &bcc,
                               AthenaArray<Real> &u) final;
  // TODO(sxc18): Implementation for AddCoordTermsDivergence_STS
  void AddCoordTermsCT(const CoordTermsDivergence &flag, const Real dt,
                       const AthenaArray<Real> &bcc, FaceField &b_out) final;
  void AddCoordTermsPar(const CoordTermsDivergence &flag, const Real &time,
                        const Real dt,
                        const AthenaArray<Real> &meshsrc, const int &npar,
                        AthenaArray<Real> &v1, AthenaArray<Real> &v2,
                        AthenaArray<Real> &v3) final;
 ...
 private:
  Real dot_a1_, dot_a2_, dot_a3_;
  Real a1_, a2_, a3_;
  Real a1_reciprocal_, a2_reciprocal_, a3_reciprocal_;
  Real a1_next_, a2_next_, a3_next_;
  Real a1_a2_, a1_a3_, a2_a3_, a1_a2_a3_;
  Real a1_ratio_, a2_ratio_, a3_ratio_;
};
