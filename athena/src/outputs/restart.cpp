...
void RestartOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool force_write) {
  ...
  for (int b = 0; b < pm->nblocal; ++b)
    sizelist[pm->my_blocks(b)->gid] = pm->my_blocks(b)->GetBlockSizeInBytes();
#ifdef MPI_PARALLEL
  // collect the size list - assuming IOWrapperSizeT is 64bit integer
  MPI_Allgatherv(MPI_IN_PLACE, mynb * sizeof(IOWrapperSizeT), MPI_BYTE,
                 sizelist, pm->nblist, pm->nslist, MPI_UINT64_T,
                 MPI_COMM_WORLD);
#endif
  ...
  for (int b=0; b<pm->nblocal; ++b) {
    ...
    os += sizeof(double);
    std::memcpy(&(idlist[os]), &(sizelist[pmb->gid]), sizeof(IOWrapperSizeT));
  ...
  for (int n = myns; n < myns + mynb; ++n) mysize += sizelist[n];
  ...
  for (int b=0; b<pm->nblocal; ++b) {
    ...
    for (int n = myns; n < pmb->gid; ++n) block_offset += sizelist[n];
    char *pdata = &(data[block_offset]);
    ...
    if (MAGNETIC_FIELDS_ENABLED) {
     ...
     if (PARTICLES > 0) pmb->ppar->PackParticlesForRestart(pdata);
  ...
  for (int n = 0; n < myns; ++n) myoffset += sizelist[n];
