...
int BoundaryBase::CreateBvalsMPITag(int lid, int bufid, int phys) {
  int val_tmp((lid << 11) | (bufid << 5) | phys);
#ifdef MPI_PARALLEL
  if (val_tmp >= MPI_TAG_UB) {
    std::stringstream msg;
    msg << "### FATAL ERROR in function [BoundaryBase::CreateBvalsMPITag]"
        << std::endl
        << "[bvals_base]: creating tag exceeds the upper boundary!"
        << std::endl;
    msg << Globals::my_rank << " " << lid << " " << val_tmp << " "
        << MPI_TAG_UB << std::endl;
    ATHENA_ERROR(msg);
  }
#endif
