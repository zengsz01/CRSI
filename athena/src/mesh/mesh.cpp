...
Mesh::Mesh(ParameterInput *pin, int mesh_test) :
  ...
  // Initialize Particles class after InitUserMeshData(pin)
  if (PARTICLES == DUST_PAR) DustParticles::Initialize(this, pin);
  if (PARTICLES == CHARGED_PAR) ChargedParticles::Initialize(this, pin);
...
Mesh::Mesh(ParameterInput *pin, IOWrapper& resfile, int mesh_test) :
  ...
  for (int n = 0; n < gids_; ++n) myoffset += offset[n];
  for (int n = gids_; n <= gide_; ++n) mysize += offset[n];
  ...
  for (int i=gids_; i<=gide_; i++) {
    ...
    for (int n = gids_; n < i; ++n) buff_os += offset[n];
    SetBlockSizeAndBoundaries(loclist[i], block_size, block_bcs);
    ...
    // check consistency
    if (offset[i] != my_blocks(i-gids_)->GetBlockSizeInBytes()) {
      msg << "### FATAL ERROR in Mesh constructor" << std::endl
          << "The restart file is broken or input parameters are inconsistent."
          << std::endl;
      ATHENA_ERROR(msg);
...
Mesh::~Mesh() {
  ...
  if (PARTICLES > 0) Particles::idmax_array.DeleteAthenaArray();
}
...
#pragma omp parallel for num_threads(nthreads)
    ...
    if (PARTICLES > 0) Particles::PostInitialize(this, pin);
...
#pragma omp parallel for num_threads(nthreads)
    for (int i=0; i<nblocal; ++i) {
      ...
      // Initialize neighbor lists in Particle class after creating all meshblocks
      if (PARTICLES > 0) pmb->ppar->LinkNeighbors();
    }
...
#pragma omp for private(pmb,pbval)
      for (int i=0; i<nblocal; ++i) {
        ...
        // Even though it is meaning less to sentd particle-mesh at the
        // initialize step, but it needed formally and would not change the
        // results
        if (PARTICLES > 0) pmb->ppar->SendParticleMesh();
      }
...
#pragma omp for private(pmb,pbval)
      for (int i=0; i<nblocal; ++i) {
        ...
        if (PARTICLES > 0) pmb->ppar->ReceiveAndSetParMeshWithWait();
...
#pragma omp parallel for num_threads(nthreads)
  for (int i = 0; i < nblocal; ++i) my_blocks(i)->NewBlockTimeStep();
...
void Mesh::ReserveMeshBlockPhysIDs() {
  ...
  if (PARTICLES > 0) {
    ReserveTagPhysIDs(ParBoundaryVariable::max_phys_id);
    ReserveTagPhysIDs(ParMeshBoundaryVariable::max_phys_id);
  }
#endif
  return;
}
