...
MeshBlock::MeshBlock(int igid, int ilid, LogicalLocation iloc, RegionSize input_block,
  ...
  if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) {
    ...
    pcoord = new ExpandingCoord(this, pin, false);
  }
  ...
  else if (PARTICLES == CHARGED_PAR)
    ppar = new ChargedParticles(this, pin);
  if (PARTICLES > 0) {
    // enroll inside the constructor of Particles
    //pbval->AdvanceCounterPhysID(ParBoundaryVariable::max_phys_id);
    pbval->AdvanceCounterPhysID(ParMeshBoundaryVariable::max_phys_id);
  }
...
MeshBlock::MeshBlock(int igid, int ilid, Mesh *pm, ParameterInput *pin,
  ...
  if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) {
    ...
    pcoord = new ExpandingCoord(this, pin, false);
    pcoord->UpdateCoords(pm->time, 0.0);
  }
  ...
  if (PARTICLES > 0) ppar->UnpackParticlesForRestart(mbdata, os);
...
std::size_t MeshBlock::GetBlockSizeInBytes() {
  ...
  if (PARTICLES > 0) size += ppar->GetSizeInBytes();
  if (SELF_GRAVITY_ENABLED)
...
//----------------------------------------------------------------------------------------
//! \fn void MeshBlock::NewBlockTimeStep()
//! \brief calculate the minimum timestep within a MeshBlock

void MeshBlock::NewBlockTimeStep() {
  this->phydro->NewBlockTimeStep();
  if (PARTICLES > 0)
    new_block_dt_ = std::min(new_block_dt_, ppar->NewBlockTimeStep());
}
