...
#ifdef MPI_PARALLEL
namespace {
MPI_Comm mhd_amr_comm(MPI_COMM_NULL), par_amr_comm(MPI_COMM_NULL);
}  // namespace
#endif
...
void Mesh::CalculateLoadBalance(double *clist, int *rlist, int *slist, int *nlist,
  ...
  for (int i=nb-1; i>=0; i--) {
    if (targetcost == 0.0 || num_mesh_threads_ * j > i) {
    ...
    mycost += clist[i];
    rlist[i] = j;
    if ((mycost >= targetcost && j > 0) || num_mesh_threads_ * j == i) {
      j--;
      totalcost -= mycost;
      mycost = 0.0;
  ...
  for (int i = 1; i < nb; i++) {  // make the list of nbstart and nblocks
    if (slist[rlist[i]] > i) {
      ...
      msg << "### FATAL ERROR in function "
          ...
          << std::endl
          << "[amr_loadbalance]: i is " << i << " rank is " << rlist[i]
          << " nbs is " << slist[rlist[i]] << std::endl;
      ATHENA_ERROR(msg);
    }
  }
...
bool Mesh::GatherCostListAndCheckBalance() {
  if (lb_manual_ || lb_automatic_) {
    ...
    if (adaptive && PARTICLES == 0)
      lb_tolerance_ = 2.0 * static_cast<double>(Globals::nranks) /
                      static_cast<double>(nbtotal);
    // If particlce module is enabled, use lb_tolerance_ from input file
...
void Mesh::RedistributeAndRefineMeshBlocks(ParameterInput *pin, int ntot) {
  ...
  int nbtold = nbtotal;
  // Additionally communicate idmax for particles
  if (PARTICLES > 0) Particles::RecordIDMax(this);
  ...
  for (int n=0; n<ntot; n++) {
    ...
    if (PARTICLES == 0) {
      if (newloc[n].level >= loclist[on].level) {  // same or refined
        ...
        double acost = 0.0;
        for (int l = 0; l < nleaf; l++) acost += costlist[on + l];
        newcost[n] = acost / nleaf;
      }
    } else {
      if (newloc[n].level == loclist[on].level) {  // same or refined
        newcost[n] = costlist[on];
      } else if (newloc[n].level > loclist[on].level) {
        newcost[n] = costlist[on] / static_cast<double>(nleaf);
      } else {
        newcost[n] = 0.0;
        for (int l = 0; l < nleaf; l++) newcost[n] += costlist[on + l];
      }
    }
  }
  ...
  MPI_Request *req_send, *req_recv, *req_send_par;
  // Step 5. allocate and start receiving buffers
  // Create new communicator
  if (mhd_amr_comm == MPI_COMM_NULL)
    MPI_Comm_dup(MPI_COMM_WORLD, &mhd_amr_comm);
  if (par_amr_comm == MPI_COMM_NULL && PARTICLES > 0) 
    MPI_Comm_dup(MPI_COMM_WORLD, &par_amr_comm);
      ...
      if (oloc.level > nloc.level) { // f2c
        for (int l=0; l<nleaf; l++) {
          ...
          MPI_Irecv(recvbuf[rb_idx], bsf2c, MPI_ATHENA_REAL, ranklist[on + l],
                    tag, mhd_amr_comm, &(req_recv[rb_idx]));
          rb_idx++;
        }
  ...
  // Step 6. allocate, pack and start sending buffers
    ...
    for (int i = 0; i < ntot; i++) {  // make the list of nbstart and nblocks
      if (nslist[newrank[i]] > i) {
        std::stringstream msg;
        msg << "### FATAL ERROR in function "
               "[Mesh::CalculateLoadBalance]"
            << std::endl
            << "[amr_loadbalance]: i is " << i << " rank is " << newrank[i]
            << " nbs is " << nslist[newrank[i]] << "in rank "<< Globals::my_rank << std::endl;
        ATHENA_ERROR(msg);
      }
    }
  ...
  if (nsend != 0) {
    ...
    if (PARTICLES > 0) {
      sendbuf_par = new char *[nsend];
#pragma ivdep
      std::fill(&sendbuf_par[0], &sendbuf_par[nsend], nullptr);
      req_send_par = new MPI_Request[nsend];
    }
    ...
    for (int n=gids_; n<=gide_; n++) {
      ...
      if (nloc.level == oloc.level) { // same level
        ...
        MPI_Isend(sendbuf[sb_idx], bssame, MPI_ATHENA_REAL, newrank[nn], tag,
                  mhd_amr_comm, &(req_send[sb_idx]));
        if (PARTICLES > 0) {
          const int size_par(Particles::PrepareSendSameLevelOrF2CAMR(pb, sendbuf_par[sb_idx]));
          MPI_Isend(sendbuf_par[sb_idx], size_par, MPI_BYTE, newrank[nn], tag,
                    par_amr_comm, &(req_send_par[sb_idx]));
        }
        sb_idx++;
        ...
        for (int l=0; l<nleaf; l++) {
          ...
          MPI_Isend(sendbuf[sb_idx], bsc2f, MPI_ATHENA_REAL, newrank[nn+l],
                    tag , mhd_amr_comm, &(req_send[sb_idx]));
          if (PARTICLES > 0) {
            const int size_par(Particles::PrepareSendC2FAMR(
                pb, sendbuf_par[sb_idx], newloc[nn + l]));
            MPI_Isend(sendbuf_par[sb_idx], size_par, MPI_BYTE, newrank[nn + l],
                      tag, par_amr_comm, &(req_send_par[sb_idx]));
          }
          sb_idx++;
        ...
        if (PARTICLES > 0) {
          const int size_par(
              Particles::PrepareSendSameLevelOrF2CAMR(pb, sendbuf_par[sb_idx]));
          MPI_Isend(sendbuf_par[sb_idx], size_par, MPI_BYTE, newrank[nn], tag,
                    par_amr_comm, &(req_send_par[sb_idx]));
        }
        sb_idx++;
    ...
    if ((ranklist[on] == Globals::my_rank) && (loclist[on].level == newloc[n].level)) {
      ...
      if ((loclist[on].level > newloc[n].level)) { // fine to coarse (f2c)
        for (int ll=0; ll<nleaf; ll++) {
          ...
          if (PARTICLES > 0) Particles::AMRFineToCoarseSameRank(pob, newlist(n - nbs));
        }
        ...
        if (PARTICLES > 0) Particles::AMRCoarseToFineSameRank(pob, newlist(n - nbs));
      }
    }
  }
        ...
        if (oloc.level > nloc.level) {  // f2c
          for (int l = 0; l < nleaf; l++) {
            ...
            int ox1 = ((lloc.lx1 & 1LL) == 1LL),
                ox2 = ((lloc.lx2 & 1LL) == 1LL),
                ox3 = ((lloc.lx3 & 1LL) == 1LL);
            int tag = CreateAMRMPITag(n - nbs, ox1, ox2, ox3);
            MPI_Probe(ranklist[on + l], tag, par_amr_comm, &status);
            MPI_Get_count(&status, MPI_BYTE, &recvsize_par);
            recvbuf_par[rb_idx] = new char[recvsize_par];
            MPI_Recv(recvbuf_par[rb_idx], recvsize_par, MPI_BYTE,
                     ranklist[on + l], tag, par_amr_comm, &status);
            if (recvbuf[rb_idx][0] == false || recvbuf[rb_idx][1] == false) {
              std::stringstream msg;
              msg << "### FATAL ERROR in function "
                     "[Mesh::RedistributeAndRefineMeshBlocks]"
                  << std::endl
                  << "[amr_loadbalance]: recieve wrong buffer from rank "
                  << ranklist[on + l] << " lid " << n - nbs << " ox: " << ox1
                  << " " << ox2 << " "
                  << " " << ox3 << std::endl;
              ATHENA_ERROR(msg);
            }
            Particles::FinishRecvAMR(pb, recvsize_par, recvbuf_par[rb_idx]);
            rb_idx++;
          }
        } else {  // same level or c2f
          if (ranklist[on] == Globals::my_rank) continue;
          int tag = CreateAMRMPITag(n - nbs, 0, 0, 0);
          MPI_Probe(ranklist[on], tag, par_amr_comm, &status);
          MPI_Get_count(&status, MPI_BYTE, &recvsize_par);
          recvbuf_par[rb_idx] = new char[recvsize_par];
          MPI_Recv(recvbuf_par[rb_idx], recvsize_par, MPI_BYTE, ranklist[on],
                   tag, par_amr_comm, &status);
          if (recvbuf[rb_idx][0] == false || recvbuf[rb_idx][1] == false) {
            std::stringstream msg;
            msg << "### FATAL ERROR in function "
                   "[Mesh::RedistributeAndRefineMeshBlocks]"
                << std::endl
                << "[amr_loadbalance]: recieve wrong buffer from rank "
                << ranklist[on] << " lid " << n - nbs << std::endl;
            ATHENA_ERROR(msg);
          }
          Particles::FinishRecvAMR(pb, recvsize_par, recvbuf_par[rb_idx]);
          rb_idx++;
  ...
  if (nsend != 0) {
    ...
    delete [] req_send;
    if (PARTICLES > 0) {
      MPI_Waitall(nsend, req_send_par, MPI_STATUSES_IGNORE);
      for (int n = 0; n < nsend; n++) delete[] sendbuf_par[n];
      delete[] sendbuf_par;
      delete[] req_send_par;
    }
  }
  if (nrecv != 0) {
    ...
    delete [] req_recv;
    if (PARTICLES > 0) {
      for (int n = 0; n < nrecv; n++) delete[] recvbuf_par[n];
      delete[] recvbuf_par;
    }
  }
#endif
...
int Mesh::CreateAMRMPITag(int lid, int ox1, int ox2, int ox3) {
  ...
  int val_tmp((lid << 8) | (ox1 << 7) | (ox2 << 6) | (ox3 << 5) | 0);
#ifdef MPI_PARALLEL
  if (val_tmp >= MPI_TAG_UB) {
    std::stringstream msg;
    msg << "### FATAL ERROR in function [Mesh::CreateAMRMPITag]" << std::endl
        << "[amr_loadbalance]: creating tag exceeds the upper boundary!"
        << std::endl;
    ATHENA_ERROR(msg);
  }
#endif
