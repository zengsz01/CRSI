...
//----------------------------------------------------------------------------------------
//! \fn template <typename T> void UnpackAndAddData(const T *buf, AthenaArray<T> &dst,
//!     int sn, int en, int si, int ei, int sj, int ej, int sk, int ek, int
//!     &offset)
//! \brief unpack a one-dimensional buffer into a 4D AthenaArray

template <typename T>
void UnpackAndAddData(const T *buf, AthenaArray<T> &dst, int sn, int en, int si,
                int ei, int sj, int ej, int sk, int ek, int &offset) {
  for (int n = sn; n <= en; ++n) {
    for (int k = sk; k <= ek; ++k) {
      for (int j = sj; j <= ej; ++j) {
#pragma omp simd
        for (int i = si; i <= ei; ++i) dst(n, k, j, i) += buf[offset++];
      }
    }
  }
  return;
...
template <typename T>
void UnpackAndAddData(const T *buf, AthenaArray<T> &dst, int si, int ei, int sj,
                int ej, int sk, int ek, int &offset) {
  for (int k = sk; k <= ek; ++k) {
    for (int j = sj; j <= ej; ++j) {
#pragma omp simd
      for (int i = si; i <= ei; ++i) dst(k, j, i) += buf[offset++];
    }
  }
  return;
}
...
template void PackData<Real>(const AthenaArray<Real> &, Real *,
                             int, int, int, int, int, int, int &);
template void UnpackAndAddData<Real>(const Real *, AthenaArray<Real> &, int,
