...
std::vector<Real> phasea, phasef, phases, ampwv, kwv;
...
int step_ran_phase; // time step between two phase randomize
  ...
  if (PARTICLES == CHARGED_PAR) {
    ...
    t_ran_phase_inv =
        1.0 / pin->GetOrAddReal("problem", "random_phase_interval",
                                lambda_max / c_tmp);
    step_ran_phase = pin->GetOrAddInteger("problem", "random_phase_step",
                                1);
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  bx0 = b_mag * std::cos(b_ang);
  by0 = 0;
  bz0 = b_mag * std::sin(b_ang);

  vx0 = -vdft * std::cos(b_ang);
  vy0 = 0;
  vz0 = -vdft * std::sin(b_ang);
  ...
  Eigensystem(den, vx0, vy0, vz0, h0, bx0, by0, bz0, xfact, yfact, ev, rem, lem);
  ...
  for (int k = ks; k <= ke; k++) {
    for (int j = js; j <= je + 1; j++) {
      for (int i = is; i <= ie; i++) {
        for (int n = 0; n < (nmax/2 - 1); ++n) {
          pfield->b.x2f(k, j, i) +=
              (ampwv[n] * rem[NWAVE-2][1] * std::sin(-kwv[n] * pcoord->x1v(i) - phasea[n])
            + ampwv[n] * rem[NWAVE-2][NWAVE-2] * std::sin(kwv[n] * pcoord->x1v(i) - phasea[n+nmax/2]));
        }
      }
    }
  }
  ...
  for (int k = ks; k <= ke + 1; k++) {
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie; i++) {
        for (int n = 0; n < (nmax/2 - 1); ++n) {
          pfield->b.x3f(k, j, i) +=
              ((ampwv[n] * rem[NWAVE-1][0] * std::sin(-kwv[n] * pcoord->x1v(i) - phasef[n])
            + ampwv[n] * rem[NWAVE-1][NWAVE-1] * std::sin(kwv[n] * pcoord->x1v(i) - phasef[n+nmax/2]))
            + (ampwv[n] * rem[NWAVE-1][2] * std::sin(-kwv[n] * pcoord->x1v(i) - phases[n])
            + ampwv[n] * rem[NWAVE-1][NWAVE-3] * std::sin(kwv[n] * pcoord->x1v(i) - phases[n+nmax/2])));
       }
     }
   }
  }
  ...
  for (int k = ks; k <= ke; k++) {
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie; i++) {
        ...
        Real mx(den * vx0), my(den * vy0), mz(den * vz0), rho(den);
        for (int n = 0; n < (nmax/2 - 1); ++n) {
          Real x = pcoord->x1v(i);

          rho += ((ampwv[n] * rem[0][1] * std::sin(-kwv[n] * x - phasea[n])
                + ampwv[n] * rem[0][NWAVE-2] * std::sin(kwv[n] * x - phasea[n+nmax/2]))
                + (ampwv[n] * rem[0][0] * std::sin(-kwv[n] * x - phasef[n])
                + ampwv[n] * rem[0][NWAVE-1] * std::sin(kwv[n] * x - phasef[n+nmax/2]))
                + (ampwv[n] * rem[0][2] * std::sin(-kwv[n] * x - phases[n])
                + ampwv[n] * rem[0][NWAVE-3] * std::sin(kwv[n] * x - phases[n+nmax/2])));

          mx += ((ampwv[n] * rem[1][1] * std::sin(-kwv[n] * x - phasea[n])
               + ampwv[n] * rem[1][NWAVE-2] * std::sin(kwv[n] * x - phasea[n+nmax/2]))
               + (ampwv[n] * rem[1][0] * std::sin(-kwv[n] * x - phasef[n])
               + ampwv[n] * rem[1][NWAVE-1] * std::sin(kwv[n] * x - phasef[n+nmax/2]))
               + (ampwv[n] * rem[1][2] * std::sin(-kwv[n] * x - phases[n])
               + ampwv[n] * rem[1][NWAVE-3] * std::sin(kwv[n] * x - phases[n+nmax/2])));

          my += ((ampwv[n] * rem[2][1] * std::sin(-kwv[n] * x - phasea[n])
               + ampwv[n] * rem[2][NWAVE-2] * std::sin(kwv[n] * x - phasea[n+nmax/2]))
               + (ampwv[n] * rem[2][0] * std::sin(-kwv[n] * x - phasef[n])
               + ampwv[n] * rem[2][NWAVE-1] * std::sin(kwv[n] * x - phasef[n+nmax/2]))
               + (ampwv[n] * rem[2][2] * std::sin(-kwv[n] * x - phases[n])
               + ampwv[n] * rem[2][NWAVE-3] * std::sin(kwv[n] * x - phases[n+nmax/2])));

          mz += ((ampwv[n] * rem[3][1] * std::sin(-kwv[n] * x - phasea[n])
               + ampwv[n] * rem[3][NWAVE-2] * std::sin(kwv[n] * x - phasea[n+nmax/2]))
               + (ampwv[n] * rem[3][0] * std::sin(-kwv[n] * x - phasef[n])
               + ampwv[n] * rem[3][NWAVE-1] * std::sin(kwv[n] * x - phasef[n+nmax/2]))
               + (ampwv[n] * rem[3][2] * std::sin(-kwv[n] * x - phases[n])
               + ampwv[n] * rem[3][NWAVE-3] * std::sin(kwv[n] * x - phases[n+nmax/2])));       
        }
...
//     if (std::floor(pmy_mesh->ncycle / step_ran_phase) !=
//         std::floor((pmy_mesh->ncycle + 1) / step_ran_phase)) {
//     std::int64_t iseed(
//         iseed_org + pmy_block->gid +
//         std::floor((pmy_mesh->ncycle + 1) / step_ran_phase));
    
//     int k_tot = int(npar * pmy_mesh->dt * step_ran_phase * t_ran_phase_inv);

//     // Similar to Boris pusher
//     Real sin_ang = std::sin(b_ang);
//     Real cos_ang = std::cos(b_ang);
//     for (int i = 0; i < k_tot; ++i) {
//       int k = int(ran2(&iseed) * npar);
