...
Real pres, gm1, b_par, vdft;
int nmax, opt;
std::int64_t iseed_org;
std::vector<Real> phasel, phaser, ampwv, kwv;
// Ion-neutral damping
Real nu_in;
// Particle properties
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  // initialize waves as a superposition of left/right circular polarized modes
  for (int i = 1; i < nmax; ++i) {
    int i1 = i <= n2 ? i : i - nmax;
    Real k_tmp(SIGN(k_min * i1) * std::min(std::abs(k_min * i1), k_max));
    kwv.push_back(k_tmp);
    phasel.push_back(2.0 * PI * ran2(&iseed));
    phaser.push_back(2.0 * PI * ran2(&iseed));
  ...
  if (PARTICLES == CHARGED_PAR) {
    ...
    p0 = pin->GetOrAddReal("problem", "p0", 300.0 * b_par / std::sqrt(den));
    ...
    for (int j = js; j <= je + 1; j++) {
      for (int i = is; i <= ie; i++) {
        for (int n = 0; n < (nmax - 1); ++n) {
          pfield->b.x2f(k, j, i) -=
              ampwv[n] * std::sin(kwv[n] * pcoord->x1v(i) - phasel[n]);
          if (opt == 1)
            pfield->b.x2f(k, j, i) -=
                ampwv[n] * std::sin(kwv[n] * pcoord->x1v(i) - phaser[n]);
        }
      }
    }
  }
  ...
  for (int k = ks; k <= ke + 1; k++) {
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie; i++) {
        for (int n = 0; n < (nmax - 1); ++n) {
          pfield->b.x3f(k, j, i) +=
              ampwv[n] * std::cos(kwv[n] * pcoord->x1v(i) - phasel[n]);
          if (opt == 1)
            pfield->b.x3f(k, j, i) -=
                ampwv[n] * std::cos(kwv[n] * pcoord->x1v(i) - phaser[n]);
        }
      }
    }
  }
  ...
  for (int k = ks; k <= ke; k++) {
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie; i++) {
        ...
        Real mx(-den * vdft), my(0.0), mz(0.0);
        for (int n = 0; n < (nmax - 1); ++n) {
          Real x = pcoord->x1v(i);
          Real snl = std::sin(kwv[n] * x - phasel[n]);
          Real csl = -std::cos(kwv[n] * x - phasel[n]);
          Real snr = std::sin(kwv[n] * x - phaser[n]);
          Real csr = std::cos(kwv[n] * x - phaser[n]);
          ...
          for (int j = 0; j < npx2_loc; ++j) {
            ...
            for (int i = 0; i < npx1_loc; ++i) {
              ...
              Real myvp(Utils::SchroderIterate(
                  ...
                  sin_mytheta(std::sin(mytheta)),
                  cos_mytheta(std::cos(mytheta));
              if (adaptive_delta_f_method_enable) {
                sin_mytheta /= beta;
                cos_mytheta /= SQR(beta);
              }

              ppar->xp(ipar) = xp1;
              ppar->yp(ipar) = yp1;
              ppar->zp(ipar) = zp1;
              ppar->vpx(ipar) = myvp * cos_mytheta;
...
Real ChargedParticles::PhaseDist(const Real &x, const Real &y, const Real &z,
  ...
  if (adaptive_delta_f_method_enable) {
    ...
    return norm_factor * a1_reciprocal * a2_reciprocal * a3_reciprocal *
           std::pow(1.0 + kappa_p0sqr_inv * (SQR(px * SQR(beta)) +
                                             SQR(py * beta) + SQR(pz * beta)),
                    -kappa - 1);
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = 0; i < ntbin * npbin; ++i) {
        ...
        if (Particles::DeltafEnable()) {
          ...
          if (adaptive_delta_f_method_enable) {
            ...
            myp_sqr *= mu_sqr * SQR(SQR(beta)) + SQR(beta) * (1.0 - mu_sqr);
  ...
  if (adaptive_delta_f_method_enable && Particles::DeltafEnable() &&
    ...
    stat_global[0] = stat_global[0] * norm + 0.5 * m_CR * SQR(p0) * kappa /
                                                 (kappa - 1.5) / SQR(SQR(beta));
    stat_global[1] = stat_global[1] * norm +
                     m_CR * SQR(p0) * kappa / (kappa - 1.5) / SQR(beta);

    beta_tmp = std::sqrt(0.5 * stat_global[1] / stat_global[0]);
    ...
    for (int i = 0; i < this->nblocal; ++i) {
      stat_local[2] +=
          this->my_blocks(i)->ppar->Statistics([beta_sqr](Real mom, Real mu) {
            return mom *
                   std::sqrt(SQR(mu) * (SQR(beta_sqr) - beta_sqr) + beta_sqr);
      });
      stat_local[3] +=
          this->my_blocks(i)->ppar->Statistics([beta_sqr](Real mom, Real mu) {
            return SQR(mom) * (SQR(mu) * (SQR(beta_sqr) - beta_sqr) + beta_sqr);
      });
    ...
    stat_global[2] =
        stat_global[2] * norm +
        m_CR * p0 * std::tgamma(kappa + 1.0) /
            (std::sqrt(PI * kappa) * (kappa - 1.0) * std::tgamma(kappa - 0.5)) *
            SQR(beta_tmp / beta) * Mean_functor(beta_tmp / beta);
    stat_global[3] = stat_global[3] * norm +
                     m_CR * kappa * SQR(p0) *
                         (1.0 + 2.0 * SQR(beta) / beta_sqr) /
                         (2.0 * kappa - 3.0) * SQR(SQR(beta_tmp / beta));
    ...
    p0 = p0_tmp;
    kappa_p0sqr_inv = 1.0 / kappa / SQR(p0);
    norm_factor = std::pow(PI * kappa * SQR(p0), -1.5) * SQR(beta_sqr) *
                  std::tgamma(kappa + 1.0) / std::tgamma(kappa - 0.5);
  }
  return;
}
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k) {
    for (int j = pmb->js; j <= pmb->je; ++j) {
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        cons(IM1, k, j, i) -= tmp * (prim(IVX, k, j, i) + vdft);
