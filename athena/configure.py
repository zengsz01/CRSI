...
#   -lprofiler        enable CPU profiler(gperftools) during compiling
#   -ltcmalloc        enable TCMalloc during compiling
# ----------------------------------------------------------------------------------------
...
parser.add_argument('--p',
                    default='none',
                    choices=['none', 'charged', 'dust'],
                    help='select particles')
...
# -lprofiler argument
parser.add_argument('-lprofiler',
                    action='store_true',
                    default=False,
                    help='enable CPU profiler(gperftools) during compiling')

# -ltcmalloc argument
parser.add_argument('-ltcmalloc',
                    action='store_true',
                    default=False,
                    help='enable TCMalloc during compiling')
...
if args['g'] and not args['t'] and args['flux'] not in ('llf', 'hlle'):
    raise SystemExit('### CONFIGURE ERROR: Frame transformations required for {0}'
                     .format(args['flux']))
if not args['g'] and args['coord'] not in ('cartesian', 'cylindrical', 'spherical_polar', 'expanding'):
...
# --p arguments
if args['p'] == "none":
    definitions['PARTICLES'] = '0'
else:
    if args['p'] == "dust":
        definitions['PARTICLES'] = '1'
    if args['p'] == "charged":
        definitions['PARTICLES'] = '2'
...
# -lprofiler argument
if args['lprofiler']:
    definitions['PROFILER_OPTION'] = 'GPERFTOOLS'
    makefile_options['LINKER_FLAGS'] += ' -lprofiler'
else:
    definitions['PROFILER_OPTION'] = 'NON_PROFILER_OPTION'

# -ltcmalloc argument
if args['ltcmalloc']:
    definitions['MALLOC_OPTION'] = 'TCMALLOC'
    makefile_options['LINKER_FLAGS'] += ' -ltcmalloc'
else:
    definitions['MALLOC_OPTION'] = 'NON_MALLOC_OPTION'
...
print('  Particles:                  ' + args['p'])
print('  Self-Gravity:               ' + self_grav_string)
print('  Super-Time-Stepping:        ' + ('ON' if args['sts'] else 'OFF'))
print('  CPU profiler flags:         ' + ('ON' if args['lprofiler'] else 'OFF'))
print('  TCMalloc flags:             ' + ('ON' if args['ltcmalloc'] else 'OFF'))
